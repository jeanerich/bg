...
<div class='panels_wrapper'>
    ...
    <div class='panels'>
    	<div class='pane upload active'>
			...
			$this->functions->upload_form("post_image", 'target1', $image_type, "registerImage", true, "select file", $user_id); ?>
        </div>
...
function registerImage(uploadData){
	var output = "<div id='library_tile_" + uploadData.image_id + "' class='tile' tile_id='" + uploadData.image_id + "' onclick='toggleTiles(" + uploadData.image_id + ");'><div class='tile_bg' style='background: url(" + uploadData.square + ") no-repeat; '></div><div class='checkbox'></div></div>";
	...
	<?php if($multiple_images): ?>
		...
		if(no_tiles > 0){
			for(var i = 0; i < no_tiles; i++){
				...
				preview_output += "<div class='tile' style=\"" + tile_style + "\" id=\"preview_tile_" + img_array[i] + "\" img_id='" + img_array[i] + "' ><a href='#' class='delete' onclick='$(this).parent().remove(); <?php if($image_type == 'office'){echo " saveAdsImages(); ";} ?> return false;'>X</a><?php if($image_type == 'office'){echo "<div class='handle'></div><div class='cover'>" . $this->functions->_e("cover image", $dictionary) . "</div>";} ?></div>";
			}
			var image_id = img_array.join(',');
		}
	<?php else: ?>
		var image_id = parseFloat($(".image_grid .tile.active").eq(0).attr('tile_id'));
	<?php endif; ?>
	<?php if($image_type == 'feed'): ?>
	$("#feed_head .feed_media.images").prepend(preview_output);
	close_fs_modal();
	$("#feed_head .feed_media.images").slideDown(250);
	<?php elseif($image_type == 'office'): ?>
	$("#photo_gallery_grid").prepend(preview_output);
	close_fs_modal();
	$("#photo_gallery_grid").slideDown(250);
	saveAdsImages();
				   ...
				   function(data){
					   if(data.success){
							...
							<?php elseif($image_type == 'battle_hero'): ?>
								var rthumb = rimage[image_id]['source'];
							$("#main_hero").attr({"style" : "background: url(" + rthumb + ") no-repeat;"});  userHelpFillProfile();
							...
							$("#profile_head #avatar_image").attr({"style" : "background: url(" + rthumb + ") no-repeat;"}); userHelpFillProfile();
